...
describe('AFIP Homologación', () => {
    ...
    beforeAll(() => {
        ...
        if (keyPath && !fs_1.default.existsSync(keyPath)) {
    ...
    describe('Configuración de Homologación', () => {
        ...
        it('debería tener archivos de certificados válidos', () => {
            ...
            const keyPath = process.env.AFIP_KEY_PATH_HOMOLOGACION;
            if (certPath) {
                expect(fs_1.default.existsSync(certPath)).toBe(true);
                expect(fs_1.default.statSync(certPath).size).toBeGreaterThan(0);
            }
            if (keyPath) {
                expect(fs_1.default.existsSync(keyPath)).toBe(true);
                expect(fs_1.default.statSync(keyPath).size).toBeGreaterThan(0);
            }
        });
    });
    describe('Emisión de Comprobante Válido', () => {
        it('debería emitir comprobante válido y recibir CAE', async () => {
            ...
            const result = await afipService_1.afipService.solicitarCAE(comprobantes_1.comprobanteHomologacion);
    ...
    describe('Control de Duplicados', () => {
        it('debería retornar CAE existente para comprobante duplicado', async () => {
            ...
            // Primera emisión
            const comprobante1 = { ...comprobantes_1.comprobanteHomologacion };
            const result1 = await afipService_1.afipService.solicitarCAE(comprobante1);
            expect(result1.cae).toBeTruthy();
            // Segunda emisión del mismo comprobante (debería retornar CAE existente)
            const comprobante2 = { ...comprobantes_1.comprobanteHomologacion };
            const result2 = await afipService_1.afipService.solicitarCAE(comprobante2);
    ...
    describe('Validación de Parámetros Inválidos', () => {
        it('debería rechazar comprobante con tipo inválido', async () => {
            ...
            const comprobanteInvalido = {
                ...comprobantes_1.comprobanteHomologacion,
                tipo: 'FACTURA_INVALIDA'
            };
            await expect(afipService_1.afipService.solicitarCAE(comprobanteInvalido))
                .rejects.toThrow();
    ...
    describe('Validación de Tiempo NTP', () => {
        it('debería validar sincronización de tiempo antes de emisión', async () => {
            ...
            const timeValidation = await afipService_1.afipService.forceTimeValidation();
