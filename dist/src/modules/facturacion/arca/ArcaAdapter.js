...
exports.validateArcaRules = validateArcaRules;
 ...
 */
function validateArcaRules(comprobante) {
    const errors = [];
    const warnings = [];
    ...
    if (!/^\d{8}$/.test(comprobante.fecha)) {
        ...
        const f = (0, dayjs_1.default)(comprobante.fecha, 'YYYYMMDD');
        if (!f.isValid()) {
...
function inferClaseComprobanteFromCondIva(cond) {
    ...
    // Mapeo conservador:
    if (cond === 'RI' || cond === 'MT' || cond === '13' || cond === '16')
        return 'A';
    if (cond === 'CF' || cond === 'EX' || cond === '7' || cond === '8' || cond === '9' || cond === '10' || cond === '15')
        return 'B';
    return null;
}
