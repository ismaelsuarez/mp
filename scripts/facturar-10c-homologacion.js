/**
 * Script: Emitir Factura C por $0,10 en HOMOLOGACIÓN (WSFEv1)
 * Uso (PowerShell):
 *   node scripts/facturar-10c-homologacion.js --cuit 20123456789 --pto 2 --cert "C:\ruta\homologacion.crt" --key "C:\ruta\homologacion.key"
 */
...
async function main() {
  console.log('🧪 Factura C de $0,10 en HOMOLOGACIÓN (WSFEv1)');
  ...
  console.log('   - Key :', key);
  if (!fs.existsSync(cert)) { console.log('❌ Certificado no encontrado'); process.exit(1); }
  if (!fs.existsSync(key)) { console.log('❌ Clave privada no encontrada'); process.exit(1); }
  try {
  ...
  // 2) Instancia AFIP (HOMOLOGACIÓN)
  console.log('\n📋 2) Creando instancia AFIP (homologación)...');
  const afip = new Afip({
    CUIT: Number(cuit),
    production: false,
    cert,
  ...
  // 3) Estado servidor (no bloqueante)
  ...
  // 4) Obtener último autorizado (puede fallar en homol si PV no corresponde)
  ...
  // 5) Enviar createVoucher CF $0,10
  ...
  try {
    ...
    if (String(msg).includes('401')) {
      console.log('   👉 401 indica WSFE homologación no habilitado o credenciales incorrectas.');
    }
    process.exit(1);
  }
}
