...
async function main() {
  ...
  // Ventana por defecto: día completo 00:00–23:59 (con soporte de parciales por TZ)
  const dateArg = process.argv[2]; // YYYY-MM-DD opcional
  const modeArg = process.argv[3]; // 'partial' | 'final' opcional
  ...
  const dateFromEnv = process.env.MP_DATE_FROM; // YYYY-MM-DD
  const dateToEnv = process.env.MP_DATE_TO;     // YYYY-MM-DD
  ...
  if (!noDateFilter) {
    if (dateFromEnv || dateToEnv) {
      // Rango por fechas (día completo en TZ)
  ...
  const payments = await searchPayments({
    begin_date,
    end_date,
    range: noDateFilter ? undefined : process.env.MP_RANGE, // si no hay filtro de fechas, también omitimos range
    status: process.env.MP_STATUS // e.g. approved,refunded
  });
  ...
  // CSV detallado con campos ampliados desde payments/search
  ...
  // CSV "full" con todas las columnas posibles (aplana el JSON)
  const flatten = (obj, prefix = '', acc = {}) => {
    if (obj === null || obj === undefined) return acc;
    if (Array.isArray(obj)) {
      // Si es array de primitivos, join; si es array de objetos, JSON
  ...
  // Generar Excel (.xlsx) a partir del "full" con tabla para fácil manipulación
  const xlsxPath = path.join(outDir, `transactions-full-${tag}.xlsx`);
  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet('Transactions');
  // Escribir encabezados
  worksheet.addRow(headers);
  // Escribir filas en el mismo orden de headers
  for (const row of normalizedRows) {
    worksheet.addRow(headers.map((h) => row[h]));
  }
  // Agregar tabla para filtros/estilo en Excel
  worksheet.addTable({
  ...
  // Generar DBF con campos curados (compatible dBase, nombres <= 10 chars)
  const dbfPath = path.join(outDir, `transactions-detailed-${tag}.dbf`);
  // Si existe de ejecuciones previas, eliminar para evitar EEXIST
  try {
